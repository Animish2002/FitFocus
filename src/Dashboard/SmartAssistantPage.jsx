...
export function SmartAssistantPage() { // Renamed component
  ...
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onend = () => {
        ...
        if (prompt.trim()) { // Check if prompt has content from voice
  ...
  const handleAskAI = async (currentPrompt = prompt) => { // Accept prompt as argument for voice input
    ...
    try {
      ...
      setError(err.response?.data?.message || err.message || "Failed to get response from AI command.");
    ...
    try {
      if (suggestedPlan.category === "Study") {
        const studyGoal = {
          ...
          category: "Study",
          description: suggestedPlan.briefOutline || `Study plan for ${suggestedPlan.topic}. Recommended duration: ${suggestedPlan.recommendedDurationMinutes} minutes.`,
      ...
      } else if (suggestedPlan.suggestedGoals && suggestedPlan.suggestedGoals.length > 0) {
        ...
        const schedulePromises = suggestedPlan.exerciseRecommendations.map(exercise => {
        ...
        setPlanAcceptedSuccess(prev => prev + " And exercises added to today's schedule!");

      } else {
        setPlanAcceptedError("No actionable plan found to add to goals/schedule.");
      }
    } catch (err) {
      console.error("Error accepting plan:", err);
      setPlanAcceptedError(err.response?.data?.message || err.message || "Failed to accept plan.");
    } finally {
      setLoading(false);
      setSuggestedPlan(null);
      setAiMessage("");
  ...
  return (
    ...
    >
      <motion.div variants={fadeInUp}>
        <h1 className="text-3xl font-bold text-white">Smart Assistant</h1> {/* Renamed title */}
        <p className="text-gray-400 mt-2">
          Command your assistant to manage tasks, get personalized plans, or ask anything.
        </p>
      </motion.div>
          ...
          <CardContent className="space-y-4">
            ...
            {isListening && (
              <p className="text-sm text-blue-400 flex items-center">
                <Volume2 className="w-4 h-4 mr-2 animate-pulse" /> {voiceInputStatus || "Listening..."}
              </p>
            )}
            {error && !isListening && ( // Show error only if not listening, as voice errors handled separately
                <p className="text-red-400 flex items-center"><XCircle className="w-4 h-4 mr-2" /> {error}</p>
            )}
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Button
                variant="outline"
                className={`flex-1 border-white/20 text-gray-300 hover:bg-white/10 hover:text-white rounded-md ${isListening ? 'bg-blue-600/20 text-blue-400' : ''}`}
                onClick={handleVoiceInputToggle}
                disabled={loading || !window.SpeechRecognition && !window.webkitSpeechRecognition} // Disable if API not supported
              >
                {isListening ? (
                  <>
      ...
      {/* AI Response Display */}
      {(aiMessage || suggestedPlan || planAcceptedSuccess || planAcceptedError) && ( // Removed 'error' from here as it's shown above
        <motion.div variants={fadeInUp}>
          <Card className="bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-xl border border-white/20 rounded-xl">
            <CardHeader>
            ...
            <CardContent>
              ...
              {planAcceptedError && <p className="text-red-400 flex items-center"><XCircle className="w-4 h-4 mr-2" /> {planAcceptedError}</p>}
              {planAcceptedSuccess && <p className="text-green-400 flex items-center"><CheckCircle className="w-4 h-4 mr-2" /> {planAcceptedSuccess}</p>}
              ...
              {suggestedPlan && (
                <div className="space-y-4 text-gray-300">
                  <h4 className="text-lg font-semibold text-white flex items-center">
                    {suggestedPlan.category === "Study" ? <CalendarDays className="w-5 h-5 mr-2 text-blue-400" /> : <Dumbbell className="w-5 h-5 mr-2 text-pink-400" />}
                    Suggested {suggestedPlan.category} Plan: {suggestedPlan.topic || suggestedPlan.summary}
                  </h4>
                  <p className="text-sm text-gray-400">{suggestedPlan.briefOutline || suggestedPlan.summary}</p>

                  {suggestedPlan.dailyCalorieIntake && (
                    <p><strong>Daily Calories:</strong> {suggestedPlan.dailyCalorieIntake} kcal</p>
                  )}
                  {suggestedPlan.recommendedDurationMinutes && (
                    <p><strong>Recommended Duration:</strong> {suggestedPlan.recommendedDurationMinutes} minutes</p>
                  )}
                  {suggestedPlan.practiceQuestions && (
                    <p><strong>Practice Questions:</strong> {suggestedPlan.practiceQuestions}</p>
                  )}
                  {suggestedPlan.difficultyLevel && (
                    <p><strong>Difficulty:</strong> {suggestedPlan.difficultyLevel}</p>
                  )}

                  {suggestedPlan.subtopics && suggestedPlan.subtopics.length > 0 && (
                    <div className="mt-2">
                  ...
                  {suggestedPlan.exerciseRecommendations && suggestedPlan.exerciseRecommendations.length > 0 && (
                    <div className="mt-2">
                      <p className="font-medium text-white">Exercise Recommendations:</p>
                      <ul className="list-disc list-inside text-sm text-gray-400 pl-4">
                        {suggestedPlan.exerciseRecommendations.map((ex, index) => (
                          <li key={index}>
                            <strong>{ex.name} ({ex.type}):</strong> {ex.frequencyPerWeek}x/week, {ex.durationMinutesPerSession} mins/session. {ex.notes}
                          </li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {suggestedPlan.dietTips && suggestedPlan.dietTips.length > 0 && (
                    <div className="mt-2">
                  ...
                  {suggestedPlan.suggestedGoals && suggestedPlan.suggestedGoals.length > 0 && (
                    <div className="mt-2">
                      <p className="font-medium text-white flex items-center">
                        <Goal className="w-4 h-4 mr-2 text-yellow-400" />Suggested Goals:
                      </p>
                      <ul className="list-disc list-inside text-sm text-gray-400 pl-4">
                        {suggestedPlan.suggestedGoals.map((goal, index) => (
                          <li key={index}>
                            <strong>{goal.name} ({goal.category}):</strong> {goal.description}
                  ...
                  <div className="mt-6 flex justify-end">
                    ...
                    >
                      {loading ? (
                        ...
                        <>
                          <CheckCircle className="mr-2 h-4 w-4" /> Accept Plan & Add to Goals
                        </>
                      )}
                    </Button>
                  </div>
